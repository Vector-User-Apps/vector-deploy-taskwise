(function() {
  ...
  window.onerror = function(msg, source, line, col, err) {
    pushLog('error', msg, source, line, col, err && err.stack);
  };
  ...
  if (origFetch) {
    window.fetch = function() {
      ...
      return origFetch.apply(this, arguments).then(function(res) {
        if (!res.ok) {
          pushLog('network', (arguments[1] && arguments[1].method || 'GET') + ' ' + url + ' ' + res.status + ' ' + res.statusText);
        }
        ...
        pushLog('network', 'FETCH FAILED ' + url + ' ' + (err && err.message || err));
        throw err;
      });
    };
  }
  ...
  XMLHttpRequest.prototype.send = function() {
    ...
    xhr.addEventListener('loadend', function() {
      if (xhr.status >= 400) {
        pushLog('network', xhr._vectorMethod + ' ' + xhr._vectorUrl + ' ' + xhr.status + ' ' + xhr.statusText);
      }
      var billingErr = xhr.getResponseHeader('X-Vector-Billing-Error');
      if (billingErr && !window.__vectorBillingErrorSent) {
        window.__vectorBillingErrorSent = true;
        window.parent.postMessage({
          type: 'VECTOR_BILLING_ERROR',
